...
To make sure that some huge pages remain available, it is recommended to limit or disable transparent hugepages by setting \path{/sys/kernel/mm/transparent_hugepage/enabled} to \texttt{madvise} or \texttt{never} (you will need to use \texttt{su} if you want to change it).
...
Dangless is a Linux static library \path{libdangless.a} that can be linked to any application during build time. It defines a set of functions for allocating and deallocating memory:
